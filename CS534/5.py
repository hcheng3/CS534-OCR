...
def main():
    ...
    ii=data_1[:,-1]==2
    iii=data_1[:,-1]==3
    print i.shape, ii.shape, iii.shape
    data=np.vstack((data_1[i], data_1[ii], data_1[iii]))
    X=data[:,:-1]
    Y=data[:,-1]
    size=X.shape[0]
    ...
    np.savez(str+"/af235", n_cluster=n_clusters_, center=af.cluster_centers_, center_labels=labels_af[af.cluster_centers_indices_])
